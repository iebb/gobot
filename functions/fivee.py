...
async def fiveEMatches(app, message, reply):

    db, cur = get_db_cur()
    cur.execute("SELECT `5e_id` FROM accounts WHERE qq = ?", (message.sender.id, ))
    row = cur.fetchone()
    if row and row[0]:
        five_e_data = get_five_e_data(row[0])
        response = [
...
async def fiveESignature(app, message, reply, data_source=0):
